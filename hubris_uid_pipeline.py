...
def balance_large_error(df, prefix):
    df_0 = df.query("le_lc == 0")
    df_1 = df.query("le_lc == 1")
...
def main(data, group, horizon, models, quantile, level):
    ...
    features_errors_uncertainty_dev["avg_interval_size_LGBM"] = (
        MAST_dev.avg_uncertainty_df["avg_interval_size_LGBM"]
    )

    features_errors_uncertainty_dev["uncertainty_quantile_LGBM"] = (
        MAST_dev.avg_uncertainty_df["uncertainty_quantile_LGBM"]
    )
    ...
    features_errors_uncertainty_dev["large_certainty"] = (
        features_errors_uncertainty_dev["unique_id"].apply(
            lambda x: 1 if x in MAST_dev.large_certainty_ids else 0
        )
    )
    ...
    conditions = [
        ...
        & (
            features_errors_uncertainty_dev["large_certainty"] == 1
        ),  # 4 - Large Error, Large Certainty
        (features_errors_uncertainty_dev["large_error"] == 0)
        & (features_errors_uncertainty_dev["large_uncertainty"] == 0),  # 0 - No Stress
        (features_errors_uncertainty_dev["large_error"] == 1)
        & (
    ...
    # target column where large errors occur in cases of large certainty
    ...
    cols_to_drop = [
        ...
        "large_certainty",
        "error_quantile_LGBM",
        "avg_interval_size_LGBM",
        "uncertainty_quantile_LGBM",
        "class",
        "le_lc",
    ]
    ...
    mm_train_le_lc_ids = [
        id
        for id in features_errors_uncertainty_dev["unique_id"][
            features_errors_uncertainty_dev["le_lc"] == 1
        ].tolist()
        if id in mm_train.index
    ]

    gen_set["le_lc"] = gen_set["unique_id"].apply(
        lambda x: 1 if x in mm_train_le_lc_ids else 0
    )
    ...
    gen_df = gen_df.drop(columns="le_lc")

    features = tsfeatures(gen_df, freq=seasonality)
    features["le_lc"] = features["unique_id"].apply(
        lambda x: 1 if x in mm_train_le_lc_ids or prefix in x else 0
    )
    ...
    features_errors_uncertainty["large_certainty"] = features_errors_uncertainty[
        "unique_id"
    ].apply(lambda x: 1 if x in mast.large_certainty_ids else 0)

    features_errors_uncertainty["avg_interval_size_LGBM"] = mast.avg_uncertainty_df[
        "avg_interval_size_LGBM"
    ]

    features_errors_uncertainty["uncertainty_quantile_LGBM"] = mast.avg_uncertainty_df[
        "uncertainty_quantile_LGBM"
    ]
    ...
    conditions = [
        ...
        & (
            features_errors_uncertainty["large_certainty"] == 1
        ),  # 4 - Large Error, Large Certainty
        (features_errors_uncertainty["large_error"] == 0)
        & (features_errors_uncertainty["large_uncertainty"] == 0),  # 0 - No Stress
        (features_errors_uncertainty["large_error"] == 1)
    ...
    features_errors_uncertainty["le_lc"] = (
        features_errors_uncertainty["class"] == 4
    ).astype(int)
